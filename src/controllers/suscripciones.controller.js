const model = require('../models/suscripciones.model');
...
controller.obtenerSuscripciones = async (req, res) => {
	try {
		const resultados = await model.obtenerSuscripciones(req.params.id);
		res.json({
			datos: resultados,
		});
	} catch (error) {
